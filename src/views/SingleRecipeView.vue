...
function addIngredient() {
  ...
  localStorage.setItem(
    "recipe",
    JSON.stringify(store.recipes.map((recipe) => recipe.getMinimalObject())),
  );
}
...
function updateSolution(multiplier) {
  ...
  store.activeRecipe.ingredients.forEach((item) => {
    const transformedString = multiplyNumberInString(
      item.textFormat,
      multiplier,
    );
...
<template>
  <div class="tabs">
    <div class="tab" @click="selectInputType('one-by-one')" :class="{ selected: inputMethod === 'one-by-one' }">
      Jedan po jedan sastojak
    </div>
    <div class="tab" @click="selectInputType('all-in-one')" :class="{ selected: inputMethod === 'all-in-one' }">
      Ceo recept odjedanput
    </div>
  </div>

  <section class="input-box" :class="inputMethod">
    <template v-if="inputMethod === 'one-by-one'">
      <input type="text" v-model="newIngredientInputValue" @keyup.enter="addIngredient" />
      <button @click="addIngredient">+</button>
    </template>

    <template v-if="inputMethod === 'all-in-one'">
      <textarea type="text" v-model="newIngredientInputValue">
 {{ store?.activeRecipe.ingredientsToTextarea() }} </textarea>
      <button @click="addIngredient">Gotovo</button>
    </template>
  </section>
  ...
  <section class="recipe">
    <div v-for="(item, index) in store?.activeRecipe.ingredients" class="recipe-item item">
      <template v-if="!item.isHeader">
        <div class="item-text">
          {{ item.textFormat }}
          <span class="item-mass" v-if="item.ingredientMass">
            ({{ item.ingredientMass }} g)</span>
        </div>
  ...
  <section class="buttons">
    <button class="button" @click="(e) => showSolution(0.25, e)">
      1/4 Četvrtina
    </button>
    <button class="button" @click="(e) => showSolution(0.33333333, e)">
      1/3 Trećina
    </button>
    <button class="button" @click="(e) => showSolution(0.5, e)">
      1/2 Pola
    </button>
    <button class="button" @click="(e) => showSolution(0.66666666, e)">
      2/3 Dve trećine
    </button>
    <button class="button" @click="(e) => showSolution(0.75, e)">
      3/4 Tri četvrtine
    </button>
    <button class="button" @click="(e) => showSolution(1.25, e)">
      1 1/4 (1 + jedna četvrtina)
    </button>
    <button class="button" @click="(e) => showSolution(1.5, e)">
      1.5x Pola više
    </button>
    <button class="button" @click="(e) => showSolution(2, e)">
      X2 Duplo
    </button>
    <button class="button" @click="(e) => showSolution(2.5, e)">
      X2.5 2 Ipo puta više
    </button>
    <button class="button" @click="(e) => showSolution(3, e)">
      X3 3 puta više
    </button>
    <button class="button" @click="(e) => showSolution(4, e)">
      X4 4 puta više
    </button>
  </section>
